...
class ZerocoinSpendTest(PivxTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zussc_bal, ussc_bal):
            zussc_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zussc_bal)
            ussc_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], ussc_bal)
            return zussc_bal, ussc_bal
        ...
        zussc_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zussc_balance, 6666)
        ...
        zussc_balance, balance = check_balances(denom_2, zussc_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zussc_balance, balance = check_balances(denom_3, zussc_balance, balance)
        ...
        _, _ = check_balances(denom_1, zussc_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
