...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_ussc, expected_zussc):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected USSC and zUSSC supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_ussc))
            for denom in node['zUSSCsupply']:
                assert_equal(node['zUSSCsupply'][denom], DecimalAmt(expected_zussc[denom]))
    ...
    def run_test(self):
        ...
        # Check USSC and zUSSC supply at the beginning
        # ------------------------------------------
        # zUSSC supply: 2 coins for each denomination
        expected_zussc_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # USSC supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zussc_supply)
        ...
        # Verify that USSC and zUSSC supplies were properly updated after the spends and reorgs
        self.log.info("Check USSC and zUSSC supply...")
        ...
        expected_zussc_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zussc_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zussc_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zussc_supply)
