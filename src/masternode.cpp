...
{
    if(nHeight <= Params().GetConsensus().treasuryheight ) return GetBlockValue(nHeight) * 0;

    return GetBlockValue(nHeight) * 05 / 100;
}

CAmount CMasternode::GetTresasuryPayment(int nHeight)
{
    if(nHeight <= Params().GetConsensus().treasuryheight ) return GetBlockValue(nHeight) * 0;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 USSC tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 USSC tx -> 1 confirmation
