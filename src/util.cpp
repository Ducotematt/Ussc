...
const char * const PIVX_CONF_FILENAME = "usscoin.conf";
const char * const PIVX_PID_FILENAME = "usscoin.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";


// UssCoin only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\usscoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\usscoin
// Mac: ~/Library/Application Support/usscoin
// Unix: ~/.usscoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UssCoin";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\usscoinParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\usscoinParams
    // Mac: ~/Library/Application Support/usscoinParams
    // Unix: ~/.usscoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UssCoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "UssCoinParams";
#else
    // Unix
    return pathRet / ".usscoin-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty usscoin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override usscoin.conf
