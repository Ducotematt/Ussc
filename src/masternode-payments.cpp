...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, const CBlockIndex* pindexPrev, bool fProofOfStake)
{
    ...
    CAmount treasuryPayment = CMasternode::GetTresasuryPayment(pindexPrev->nHeight + 1);
        ...
        if (fProofOfStake) {
            ...
            if (!txNew.vout[1].IsZerocoinMint()) {
                if (i == 2) {
                    ...
                    if(treasuryPayment > 0){
                    	CAmount treasurySplit = treasuryPayment / outputs;
	                    CAmount trPaymentRemainder = treasuryPayment - (treasurySplit * outputs);
	                    for (unsigned int j=1; j<=outputs; j++) {
	                        txNew.vout[j].nValue -= treasurySplit;
	                    }
	                    txNew.vout[outputs].nValue -= trPaymentRemainder;
                    }
                }
            }
        } else {
            ...
            if(treasuryPayment > 0){
	            ...
	            txNew.vout[0].nValue = CMasternode::GetBlockValue(pindexPrev->nHeight + 1) - masternodePayment - treasuryPayment;
            }
    	...
    	if(treasuryPayment > 0){
    	 if (fProofOfStake) {
            ...
            if (!txNew.vout[1].IsZerocoinMint()) {
    	 	...
    	 	txNew.vout[0].nValue = CMasternode::GetBlockValue(pindexPrev->nHeight + 1)  - treasuryPayment;
    	 }
    	}
    }
}
