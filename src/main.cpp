...
#if defined(NDEBUG)
#error "UssCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uUSSC) are considered zero fee (for relaying, mining and transaction creation)
                              ...
                              std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            // Reject legacy zUSSC mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zUSSC mint output in tx %s",
...
DisconnectResult DisconnectBlock(CBlock& block, CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    //Track zUSSC money supply
    if (!UpdateZPIVSupplyDisconnect(block, pindex)) {
        error("%s: Failed to calculate new zUSSC supply", __func__);
        return DISCONNECT_FAILED;
    }
...
{
    util::ThreadRename("ussc-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    // Update zUSSC money supply map
    if (!UpdateZPIVSupplyConnect(block, pindex, fJustCheck)) {
        return state.DoS(100, error("%s: Failed to calculate new zUSSC supply for block=%s height=%d", __func__,
                                    block.GetHash().GetHex(), pindex->nHeight), REJECT_INVALID);
    }

    // A one-time event where the zUSSC supply was off (due to serial duplication off-chain on main net)
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (unsigned int i = 1; i < block.vtx.size() - 1; i++)
    ...
    if (IsPoS) {
        ...
        for (unsigned int i = 2; i < block.vtx.size() - 1; i++)
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zUSSC spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zUSSC serial %s in block\n Block: %s",
    ...
    if (isPoS) {
        ...
        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                ...
                usscInputs.push_back(stakeIn);
            }
        }
        const bool hasPIVInputs = !usscInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasPIVInputs) {
                    ...
                    for (const CTxIn& usscIn : usscInputs)
                        if(usscIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasPIVInputs) {
                            for (const CTxIn& stakeIn : usscInputs)
            ...
            // Now that this loop if completed. Check if we have zUSSC inputs.
            if(hasZPIVInputs) {
                for (const CTxIn& zPivInput : zPIVInputs) {
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zUSSC");
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // UssCoin: We use certain sporks during IBD, so check to see if they are
