...
void PrepareShutdown()
{
    ...
    util::ThreadRename("ussc-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all USSC specific functionality (Masternodes, Zerocoin, Budgeting) (0-1, default: %u)"), 0));
...
{
    util::ThreadRename("ussc-loadblk");
...
/** Sanity checks
 *  Ensure that UssCoin is running in a usable environment with all
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. UssCoin is shutting down."));

    std::string strDataDir = GetDataDir().string();

    // Make sure only a single UssCoin process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. UssCoin is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //UssCoin specific: zerocoin and spork DB's
                ...
                // UssCoin: load previous sessions sporks if we have them.
                ...
                if (fReindexMoneySupply) {
                    LOCK(cs_main);
                    // Skip zUSSC if already reindexed
