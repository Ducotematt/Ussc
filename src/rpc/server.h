...
extern UniValue DoZusscSpend(const CAmount nAmount, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str);
...
extern UniValue setzusscseed(const JSONRPCRequest& request);
extern UniValue getzusscseed(const JSONRPCRequest& request);
extern UniValue generatemintlist(const JSONRPCRequest& request);
extern UniValue searchdzussc(const JSONRPCRequest& request);
extern UniValue dzusscstate(const JSONRPCRequest& request);
