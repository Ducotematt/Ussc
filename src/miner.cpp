...
{
    // double check that there are no double spent zUSSC spends in this block or tx
    if (tx.HasZerocoinSpendInputs()) {
        ...
        //This zUSSC serial has already been included in the block, do not add this tx.
        if (fDoubleSerial) {
            return false;
        }
    }
    ...
    // Collect memory pool transactions into the block
    {
        ...
        for (CTxMemPool::indexed_transaction_set::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zUSSC spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // zUSSC check to not include duplicated serials in the same block.
        ...
        if (!fProofOfStake) {
        	...
        	if( nFees != 0 ){
			    ...
			    feeTx.vin.resize(1);
			    feeTx.vin[0].scriptSig = CScript() << pindexPrev->nHeight + 1 << OP_0;
			    feeTx.vout.resize(1);
			    feeTx.vout[0].scriptPubKey = treasuryPayee;
			    feeTx.vout[0].nValue = nFees;
			    
			    pblock->vtx.push_back(feeTx);
        	}
        }
