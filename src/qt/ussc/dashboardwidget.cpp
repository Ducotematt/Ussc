...
#include "qt/ussc/dashboardwidget.h"
#include "qt/ussc/forms/ui_dashboardwidget.h"
#include "qt/ussc/sendconfirmdialog.h"
#include "qt/ussc/txrow.h"
    ...
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of USSC earned via Staking & Masternodes"));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquarePiv, "square-chart-ussc");
    setCssProperty(ui->labelPiv, "text-chart-ussc");
    ...
    setCssProperty(ui->labelAmountPiv, "text-stake-ussc-disable");
	ui->labelAmountMNRewards->setText("0 USSC");
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("USSC")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair USSC, zUSSC
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            ussc = (pair["ussc"] != 0) ? pair["ussc"] / 100000000 : 0;
            mnrewards = (pair["mn"] != 0) ? pair["mn"] / 100000000 : 0;
            chartData->totalPiv += pair["ussc"];
            chartData->totalMNRewards += pair["mn"];
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesPiv.append(ussc);    
        chartData->valuesMNRewards.append(mnrewards);    

        int max = std::max(ussc, mnrewards);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    if (chartData->totalPiv > 0 || chartData->totalMNRewards > 0) {
        setCssProperty(ui->labelAmountPiv, "text-stake-ussc");
