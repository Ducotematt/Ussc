...
#include "qt/ussc/defaultdialog.h"
#include "qt/ussc/settings/settingsfaqwidget.h"
...
PIVXGUI::PIVXGUI(const NetworkStyle* networkStyle, QWidget* parent) :
    ...
    if (windowTitle.isEmpty()) {
        windowTitle = tr("USSC Core Wallet") + " || ";
        windowTitle += ((enableWallet) ? tr("Global Smart Asset") : tr("Node"));
    }
...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("UssCoin client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("UssCoin"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("UssCoin"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
