...
- [Building PIVX Core](#building-ussc-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/ussc-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/ussc-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/ussc-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/ussc-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/ussc-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/ussc-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/ussc-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@ussc.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/ussc-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/ussc-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/ussc-osx-*-build.assert
```
